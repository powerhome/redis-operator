...
While rolling out multi-site data replication we discovered that sentinels picking up the "salve" HAProxy pod IP address from the redis INFO command and treats them as valid Redis replica nodes. The sentinels will then consider the HAProxy pods as candidates for "master" promotion during a failover. Since this is not a valid IP address of redis, the operator and sentinel get stuck in a leader election loop.
